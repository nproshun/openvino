...
JitConstants KernelBase::MakeBaseParamsJitConstants(const base_params& params, bool add_tensor_definitions) const {
    ...
    auto activationType = GetComputeDatatype(params.outputs[0].GetDType());
    jit.Merge(MakeActivationJitConstants(params.activations, activationType, "", false, false, convert_input_to_output_dt));
